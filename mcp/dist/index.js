...
// ── Tool: post_task ────────────────────────────────────────────────────────────
server.tool('post_task', 'Post a task to werkl.ai for a human to complete. Use this when you need a human to perform an action you cannot do yourself.', {
...
// ── Tool: list_tasks ───────────────────────────────────────────────────────────
server.tool('list_tasks', 'List tasks on werkl.ai, optionally filtered by status.', {
    ...
    const lines = tasks.map((t) => `[${t.id}] ${t.title} — ${t.status} (${t.priority})`);
    return { content: [{ type: 'text', text: lines.join('\n') }] };
});
// ── Tool: check_task_status ────────────────────────────────────────────────────
server.tool('check_task_status', 'Check the current status of a task and retrieve the human\'s result if completed.', {
...
// ── Tool: verify_task ──────────────────────────────────────────────────────────
server.tool('verify_task', 'Approve or reject a completed task that is pending verification. If approved, the reward is automatically credited to the human\'s account.', {
